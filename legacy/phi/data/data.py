...
class Frame(DataChannel):
    ...
    def get(self, datasource, indices):
        return [np.expand_dims(i, 0) for i in indices]
    ...
    def multi_lookup(self, indices):
        try:
            ...
            for field in self.fields:
                values = 'not retrieved'
                try:
                    values = list(self.lookup_table.lookup(field, indices))
                    np.concatenate(values)
                except:
                    raise ValueError("Error in data lookup of field %s at indices %s, values=%s" % (field, indices, values))
