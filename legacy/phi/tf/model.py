...
class TFModel(FieldSequenceModel):
    ...
    def editable_bool(self, name, initial_value):
        val = EditableBool(name, initial_value)
        setattr(self, "bool_"+name.lower(), val)
        placeholder = tf.placeholder(tf.float32, (), name.lower())
        self.add_scalar(name, placeholder)
        self.editable_placeholders[placeholder] = "bool_"+name.lower()
        return placeholder
