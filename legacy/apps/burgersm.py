...
class BurgerSM(TFModel):
    def __init__(self):
        TFModel.__init__(self, "BurgerSM %d"%n, "Slow motion density generation CNN training",
        ...
        # supervised_models = {  # without force
    ...
    def setup(self):
        ...
        # Simulation
        self.sim = TFFluidSimulation([32], "open", 16)
    ...
    def action_write_forces(self):
        baseline = self.view(self.baseline_force)
        total = self.view(self.total_force)
        print("Baseline")
        print(baseline)
        print("Actual")
        print(total)
