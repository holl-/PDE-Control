...
def ik(sim, initial_density, target_density, trainable=True):
...
class GraphBuilder(PartitioningExecutor):
    ...
    def partition(self, n, initial_frame, target_frame, center_frame):
        ...
        # center_frame.density = self.run_sm(n, initial_frame.density, target_frame.density)  # TODO use this if SM
        center_frame.density = center_frame.true
...
class SmokeSM(TFModel):
    def __init__(self):
        TFModel.__init__(self, "Refine-SmokeIK %d"%n, "Slow motion density generation CNN training",
                         learning_rate=1e-6, data_validation_fraction=0.1,
                         training_batch_size=0, validation_batch_size=0,
                         model_scope_name="ik")
        ...
        sm_supervised_squares = "/home/holl/model/refine-smokesm-32/sim_000004/checkpoint_00006811"
        sm_refined_shapes = "/home/holl/model/refine-smokesm-16/sim_000015/checkpoint_00286271"

        ik_single_frame = "/home/holl/model/indirect-smokeik/sim_000054/checkpoint_00002700"
        ik_4_frame = "/home/holl/model/refine-smokeik-4/sim_000004/checkpoint_00000432"
        ik_16_frame = "/home/holl/model/refine-smokeik-16/sim_000009/checkpoint_00043426"
        self.executor.load_all_from(n, ik_16_frame, sm_supervised_squares, lambda i: 2)
    ...
    def setup(self):
        # Simulation
        self.sim = TFFluidSimulation([128] * 2, DomainBoundary([(False, True), (False, False)]), force_use_masks=True, batch_size=100)
        ...
        if use_graph:
            self.executor = GraphBuilder(self.sim, true_densities, trainable_n=[], ik_trainable=True, info=self.info, force_inference=force_inference)
        else:
        ...
        if use_graph:
            ...
            force_losses = []
            for frame in seq:
                density_losses.append(l2_loss(blur(frame.true - frame.real, 2.5, cutoff=5)))  # pred[-1]
            ...
            density_loss = math.add(density_losses) * self.editable_float("Density_Loss_Scale", 1e-1)
            force_loss = 0
            # force_loss = tf.add_n(force_losses) * self.editable_float("Force_Loss_Scale", 1e-5)
            self.add_scalar("Total_Density_Loss", density_loss)
            # self.add_scalar("Total_Force_Loss", force_loss)
            self.unsupervised_optim = self.minimizer("Unsupervised_Loss", force_loss + density_loss)
    ...
    def action_plot_sequences(self):
        ...
        real_velocities = [self.executor.lookup(self.sequence[i].velocity) for i in range(len(self.sequence)-1)]
        if use_graph:
        ...
        np.save(self.scene.subpath("seq_vel"), np.stack([v.staggered for v in real_velocities], 1))
