...
class SmokeSM(TFModel):
    def __init__(self):
        ...
        self.info("Time Step: %s" % ("normal" if not half_res else "half_res"))
        ...
        for i in range(n+1):
            self.info("BatchSelect for i=%d: %s" % (i, range(fac*i, 65-fac*n+fac*i)))
        ...
        self.database.put_scenes(scenes('~/data/SmokeIK', 'random3p_128x128'), per_scene_indices=range(4, 4+fac*n+1), logf=self.info)  # Something in this dataset causes NaN for small n
    ...
    def setup(self):
        ...
        if use_graph:
            ...
            final_density_loss = l2_loss(self.blurred_density_diff) * self.editable_float("Final_Density_Loss_Scale", 1e1) # 1e7 for 1/8 res 4px, 1e4 for 4px
